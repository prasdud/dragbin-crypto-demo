...
export async function decryptPrivateKey(encryptedPrivateKey, password, salt, iv) {
    ...
    // Decrypt private key
    const decryptedData = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: iv }, key, encryptedPrivateKey);
...
export async function decryptFile(encryptedData, password, encryptedPrivateKey, salt, iv) {
    ...
    const sessionKey = await crypto.subtle.importKey('raw', rawSessionKey, { name: 'AES-GCM' }, false, ['decrypt']);
    ...
    while (offset < encryptedData.length) {
        ...
        // Decrypt chunk
        try {
            const decryptedChunk = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: chunkIv }, sessionKey, encryptedChunk);
