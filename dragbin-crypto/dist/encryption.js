 ...
 */
import { kyberEncapsulate } from './kyber.js';
import { deriveKeyFromPassword } from './keyDerivation.js';
import { generateIV, concatUint8Arrays, numberToBytes, bytesToBase64, } from './utils.js';
/**
...
async function generateSessionKey() {
    return await crypto.subtle.generateKey({ name: 'AES-GCM', length: 256 }, true, // extractable
    ['encrypt', 'decrypt']);
}
/**
...
export async function encryptFile(file, publicKey) {
    ...
    const sessionKey = await crypto.subtle.importKey('raw', new Uint8Array(rawSessionKey), { name: 'AES-GCM' }, false, ['encrypt']);
    // Create metadata
    const metadata = {
    ...
    while (offset < file.length) {
        ...
        const iv = generateIV();
        // Encrypt chunk
        const encryptedChunk = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: iv }, sessionKey, chunk);
...
export async function encryptPrivateKey(privateKey, password) {
    ...
    const iv = generateIV();
    // Encrypt private key
    const encryptedData = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: iv }, key, privateKey);
    return {
