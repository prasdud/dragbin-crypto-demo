...
import { FileLock, Key, Download, X, Server } from 'lucide-react';
    ...
    return (
        <>
            ...
            <Button
                ...
                size="icon"
                className="fixed bottom-6 right-6 z-[60] lg:hidden h-12 w-12 rounded-full shadow-xl neon-glow border border-primary/50"
            >
                {isOpen ? <X className="w-5 h-5" /> : <Server className="w-5 h-5" />}
            </Button>
            ...
            {/* Sidebar */}
            <div className={`h-full border-l border-border bg-card/95 backdrop-blur-xl w-[320px] sm:w-[350px] flex flex-col fixed right-0 top-0 bottom-0 z-50 transition-transform duration-300 shadow-2xl overflow-hidden transform ${isOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`}>
                <div className="p-4 border-b border-border bg-background/90 z-10 flex justify-between items-start">
                    <div>
                    ...
                    <Button variant="ghost" size="icon" className="lg:hidden -mt-1 -mr-2 text-muted-foreground hover:text-foreground" onClick={() => setIsOpen(false)}>
                        <X className="w-4 h-4" />
                    </Button>
                </div>
